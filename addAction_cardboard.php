...
if (isset($_POST['submit'])) {
	...
	if (empty($reference) || empty($agency) || empty($length) || empty($width) || empty($height) || empty($cardboard_type) || empty($cardboard_replacement) || empty($cardboard_notes)) {
		...
		$result = mysqli_query($conn, "INSERT INTO cardboard (`reference`, `agency`, `length`, `width`, `height`, `cardboard_type` , `cardboard_replacement` , `cardboard_notes`) VALUES ('$reference', '$agency', '$length', '$width' , '$height' , '$cardboard_type' , '$cardboard_replacement' , '$cardboard_notes')");
