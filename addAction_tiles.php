...
if (isset($_POST['submit'])) {
	...
	if (empty($reference) || empty($agency) || empty($broken_tiles) || empty($surface_uneven) || empty($damage_water) || empty($tiles_replace) || empty($tile_damage) || empty($length) || empty($width) || empty($height) || empty($add_notes)) {
		...
		$result = mysqli_query($conn, "INSERT INTO tiles (`reference`, `agency`, `broken_tiles`, `surface_uneven`, `damage_water`, `tiles_replace` , `tile_damage` , `length` , `width` , `height` , `add_notes`) VALUES ('$reference', '$agency', '$broken_tiles', '$surface_uneven' , '$damage_water' , '$tiles_replace', '$tile_damage', '$length', '$width', '$height', '$add_notes')");
