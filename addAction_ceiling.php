...
if (isset($_POST['submit'])) {
	...
	if (empty($reference) || empty($agency) || empty($ceiling_intact) || empty($ceiling_type) || empty($water_damage) || empty($spots_stains_mold) || empty($ceiling_note)) {
        ...
        if (empty($ceiling_note)) {
			echo "<font color='red'>ceiling_note field is empty.</font><br/>";
		}
		...
		$result = mysqli_query($conn, "INSERT INTO ceiling (`reference`, `agency`, `ceiling_intact`, `ceiling_type`, `water_damage`, `spots_stains_mold` , `ceiling_note`) VALUES ('$reference', '$agency', '$ceiling_intact', '$ceiling_type' , '$water_damage' , '$spots_stains_mold','$ceiling_note')");
