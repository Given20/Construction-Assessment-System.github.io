...
if (isset($_POST['update'])) {
	...
	if (empty($reference) || empty($agency) || empty($length) || empty($width) || empty($height) || empty($damages) || empty($cracks) || empty($notes)) {
		...
		$result = mysqli_query($conn, "UPDATE foundation SET `reference` = '$reference', `agency` = '$agency', `length` = '$length', `width` = '$width', `height` = '$height' , `damages` = '$damages' , `cracks` = '$cracks', `notes` = '$notes' WHERE `id` = $id");
