...
if (isset($_POST['update'])) {
    ...
    $damage_cause = mysqli_real_escape_string($conn, $_POST['damage_cause']);
    $replacement = mysqli_real_escape_string($conn, $_POST['replacement']);
    $notes = mysqli_real_escape_string($conn, $_POST['notes']);
	// Check for empty fields
	if (empty($reference) || empty($agency) || empty($length) || empty($width) || empty($height) || empty($damage_cause) || empty($replacement) || empty($notes)) {
		if (empty($reference)) {
        ...
        if (empty($damage_cause)) {
			echo "<font color='red'>Damages field is empty.</font><br/>";
		}
        if (empty($replacement)) {
			echo "<font color='red'>Replacement field is empty.</font><br/>";
		}
        if (empty($notes)) {
		...
		$result = mysqli_query($conn, "UPDATE electrical SET `reference` = '$reference', `agency` = '$agency', `length` = '$length', `width` = '$width', `height` = '$height' , `damage_cause` = '$damage_cause' , `replacement` = '$replacement', `notes` = '$notes' WHERE `id` = $id");
		
		// Display success message
		echo "<p><font color='green'>Data added successfully!</p>";
		echo "<a href='electrical_table.php' class='btn btn-primary'>View Result</a>";
	}
}
?>
</body>
